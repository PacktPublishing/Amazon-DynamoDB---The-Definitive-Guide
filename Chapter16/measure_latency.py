...
put_items()
end_time = time.time()
logger.info("Time to insert data: {} seconds at {} Writes per second"
            .format(end_time - start_time, NUM_WRITES / (end_time - start_time)))
...
def measure_latency(client, name):
    ...
    start_time = time.time()
    threads = [threading.Thread(target=get_items, args=(client, i, latencies, op_count_list)) for i in
               range(NUM_THREADS)]
